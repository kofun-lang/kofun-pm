...
| P2 manifest & lock | dependency surface, lock v1 resolution pinning, re-lock idempotence, and four named refusals — **landed**; v0.5.0 adds bounded lock-v2/store inspection and v0.6.0 adds strict metadata parsing plus the selected descriptor bijection, while the writer, catalog/graph binding, manifest/tool identity checks, and migration remain | [#14](https://github.com/kofun-lang/kofun-pm/issues/14), which supplies authenticated metadata history and graph acquisition; manifest binding remains [#5](https://github.com/kofun-lang/kofun-pm/issues/5) |
